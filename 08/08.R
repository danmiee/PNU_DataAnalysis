...
library(dplyr)
# 시계열 분석에서 원칙적으로 나눌 필요없음 / 학습의 용이성을 위하여 추가한 과정
train <- df %>% filter(일시 < '2022-01-01' & 일시 >= '2020-01-01')
test <- df %>% filter(일시 >= '2022-01-01')

#1. 데이터준비
temp <- train $평균기온  # vector

#2. 시계열 데이터 생성 
temp_ts <- ts(temp, frequency = 12, start = c(2020, 1))

#3. 시계열추세 확인 - 시계열 분석은 주기성을 갖는 데이터에 사용 가능
#추세 (Trend): 데이터의 장기적인 증가 또는 감소를 추세라고 한다.
              #그것은 반드시 선형적인 것은 아니다.
              #그것은 시간의 경과에 따른 데이터의 기본 패턴이다. 
#계절성 (Seasonal): 시리즈가 계절적 요인에 의해 영향을 받는 경우
                  #(즉, 분기, 월 또는 요일) 시리즈에는 계절성이 존재한다.
                  #그것은 항상 고정되고 알려진 기간이다.
  #예: -크리스마스 기간 동안 매출이 급 상승하는 경우 등.
#주기 (Cyclic): 일정 기간이 아닌 데이터가 상승하거나 하락할 때
              #우리는 이를 주기적 패턴이라고 부른다.
  #예를들어 - 이러한 변동 지속기간은 일반적으로 최소 2년이다. 
...
arima  # ARIMA(3,0,0) with non-zero mean 
